...
int main(){
    ...
    Shader shader(std::string("src/shader/blending.vert"), std::string("src/shader/blending.frag"));
    ...
    unsigned int transparentTexture = loadTexture(FileSystem::getPath("res/textures/window.png").c_str());
    ...
    while(!glfwWindowShouldClose(window)){
		...
		/*在开启透明度混合已经深度测试之后，需要保证不透明物体先渲染，再到透明物体按与摄像机的距离从远到近顺序渲染*/
		/*不然深度测试会直接丢弃深度较后的fragment导致混合异常*/
        //shader 参数
		...
		for (auto iter = sorted.rbegin();iter != sorted.rend();++iter) {
            shader.setMat4("model", glm::translate(model, windows[iter->second]));
